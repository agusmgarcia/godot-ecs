...
Implement this interface (instead of bare `IComponent`) when your component wraps a single observable value. `ComponentGenerator` automatically emits `Value` and `ValueChanged` for any class that lists `IComponent<TValue>` directly, regardless of which Godot base class the component extends.
...
A convenient abstract base for components that can inherit from `Component` (a `Godot.Node`) and wrap a single value. Extends `Component` and implements `IComponent<TValue>`; `Value` and `ValueChanged` are generated by `ComponentGenerator`.
